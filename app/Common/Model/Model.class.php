...
class Model extends \Think\Model {
    ...
    protected function _facade($data) {
        ...
        if (!empty($this->fields)) {
            ...
            if ($this->isNewRecord === true) {
                ...
                $structure = $this->get_structure($this->getTableName());
                foreach ($fileList as $k => $f) {
                    if (!isset($data[$f]) && $f != $fields['_pk']) {
                        $this->setDefault($data, $structure, $f);
                    }
                }
     ...
     * @param $type
     */
    protected function setDefault(&$data, $structure, $field) {
        if (isset($structure[$field])) {
            $default = $structure[$field]['default'];
            if (strlen($default) > 0) {
                $data[$field] = $default;
            } else {
                //没有默认值，检查类型
