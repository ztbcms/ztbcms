...
class PositionController extends AdminBase {
    ...
    public function rebuilding() {
        ...
        if ($action == 0) {
            ...
            if ($action == 1) {
                ...
                $dataList = $model->where(array(
                    'posid' => $posid,
                    'id' => array('GT', $cid)
                ))->order(array('id' => 'ASC'))->limit(20)->select();
                if (empty($dataList)) {
                    $this->success('数据重建完毕！', U('index'));
                    exit;
                }
                ...
                $this->assign('waitSecond', 100);
                $this->success("数据重建中..总共<font color=\"#FF0000\">{$lun}</font>轮，当前第<font color=\"#FF0000\">{$i}</font>轮，进度：<font color=\"#FF0000\">{$jindu}%</font>",
                    U('rebuilding', $_GET));
            }
        }
    }
    ...
    public function item() {
        if (IS_POST) {
            ...
            $data = $db->where($where)->order(array(
                "listorder" => "DESC",
                "id" => "DESC"
            ...
            $this->assign("Page", $page->show())->assign("data", $data)->assign("posid", $posid)->assign('menuReturn',
                    array('url' => U('index'), 'name' => '返回推荐位管理'))->display();
        }
    }
        ...
        if (IS_POST) {
            ...
            foreach ($_POST['listorders'] as $_k => $listorder) {
                ...
                $db->where(array(
                    ...
                    'catid' => $pos[0],
                    'posid' => $_POST['posid']
                ))->data(array('listorder' => $listorder))->save();
            }
            $this->success("排序更新成功！");
        } else {
