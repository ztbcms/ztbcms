...
class ContentController extends AdminBase {
    ...
    protected function _initialize() {
        ...
        if (User::getInstance()->isAdministrator() !== true) {
            ...
            if (strpos(ACTION_NAME, 'public_') === false && ACTION_NAME != 'index') {
                ...
                $priv_datas = M('CategoryPriv')->where(array(
                    'catid' => $this->catid,
                    'is_admin' => 1,
                    'roleid' => User::getInstance()->role_id,
                    'action' => $action
                ))->find();
                if (empty($priv_datas)) {
    ...
    public function classlist() {
        ...
        $this->assign($catInfo)->assign('Page', $page->show())->assign('catid', $this->catid)->assign('count',
                $count)->assign('data', $data);
...
//			}
    ...
    public function public_relationlist() {
        ...
        if (empty($modelid)) {
            ...
            $count = $model->where($where)->count();
            $page = $this->page($count, 12, I('page', 1));
                ...
                case 0:
                    if ($ids) {
                        ...
                        foreach ($ids as $sid) {
                            ...
                            $model->where(array('catid' => $catid, 'id' => $sid))->save(array(
                                'catid' => $tocatid,
                                'url' => $urls['url']
                            ));
                            //更新推荐位
    ...
    public function public_getsite_categorys() {
        ...
        if (User::getInstance()->isAdministrator() !== true) {
            ...
            $priv_result = $this->priv_db->where(array(
                'action' => 'add',
                'roleid' => User::getInstance()->role_id,
                'is_admin' => 1
            ))->select();
    ...
    public function push() {
        if (IS_POST) {
                ...
                case 'position_list':
                    ...
                    if (!empty($position)) {
                        ...
                        foreach ($position as $_key => $_value) {
                            ...
                            //如果设置了模型，又设置了栏目
                            if ($_value['modelid'] && $_value['catid'] && !in_array($catid,
                                    explode(',', $_value['catid']))
                            ) {
                                continue;
                            }
                            //如果设置了栏目
            ...
            $this->assign('id', $id)->assign('action', $action)->assign('modelid', $modelid)->assign('catid',
                    $catid)->assign('show_header', true)->display($tpl);
        }
    }
    ...
    public function public_othors() {
        $catid = I('get.catid', 0, 'intval');
        $this->assign('catid', $catid)->display('add_othors');
    }
