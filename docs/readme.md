# 文档索引

## 入门指南

- [安装框架](./GetStart.md)
- [菜单](./Menu.md)
- [常见问题](./Faq.md)

## 组件文档

- [KV 组件](./KVComponent.md) - 键值对存储组件，支持缓存和持久化
- [Lock 组件](./LockComponent.md) - 基于 KV 组件的分布式锁实现
- [Message 组件](./MessageComponent.md) - 消息定义、发送器编排与发送日志管理
- [队列任务组件](./QueueComponent.md) - 队列任务的使用方法和开发规范
- [计划任务（Cron）组件](./CronComponent.md) - 计划任务调度与脚本开发规范
- [下载中心（Downloader）组件](./DownloaderComponent.md) - 远程文件下载、失败重试与超时治理
- [上传组件](./UPLOAD_GUIDE.md) - 上传组件的使用方法和开发规范

## 开发指南

- [起步](./GetStart.md)
- [后端开发指南](./backend.md)
- [前端与视图交互](./frontend.md)
- [模块开发](./ModuleDevelopment.md)
- [路由开发指南](./RouteGuide.md)
- [数据库迁移（Migrate）](./MigrateGuide.md)
- [php think 命令速查](./ThinkCommand.md) - 代码生成、迁移、队列、计划任务等 CLI 命令速查
